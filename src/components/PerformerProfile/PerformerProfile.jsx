...
// ✅ FIXED Gallery Component with Video Support
const Gallery = ({ images = [], videos = [] }) => {
  const [selectedMedia, setSelectedMedia] = useState(null);
  const [currentIndex, setCurrentIndex] = useState(0);

  // ✅ Enhanced video detection function that supports multiple formats
  const isVideo = (url) => {
  ...
  // ✅ Helper function to get correct MIME type for videos
  ...
  // ✅ Render individual media item with proper video detection
  const renderMediaItem = (item, index) => {
    if (isVideo(item)) {
      return (
        ...
        >
          <source src={item} type={getVideoMimeType(item)} />
  ...
  // ✅ Render modal media with proper video detection
  const renderModalMedia = (media) => {
    if (isVideo(media)) {
      return (
        <video 
          controls 
            ...
            {allMedia.length > 1 && (
              <>
                <button
                  ...
                  className="absolute left-4 top-1/2 transform -translate-y-1/2 w-12 h-12 bg-black bg-opacity-50 rounded-full flex items-center justify-center text-white hover:bg-opacity-70 transition-all"
                >
                  <svg width="24" height="24" viewBox="0 0 24 24" fill="none">
                ...
                <button
                  ...
                  className="absolute right-4 top-1/2 transform -translate-y-1/2 w-12 h-12 bg-black bg-opacity-50 rounded-full flex items-center justify-center text-white hover:bg-opacity-70 transition-all"
                >
                  <svg width="24" height="24" viewBox="0 0 24 24" fill="none">
